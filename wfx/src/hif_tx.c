...
int wfx_hif_join(struct wfx_vif *wvif, const struct ieee80211_vif_cfg *vifconf,
const struct ieee80211_bss_conf *bssconf,
		 struct ieee80211_channel *channel, const u8 *ssid, int ssidlen)
{
	...
	WARN_ON(!bssconf->beacon_int);
	WARN_ON(!bssconf->basic_rates);
	WARN_ON(sizeof(body->ssid) < ssidlen);
	WARN(!vifconf->ibss_joined && !ssidlen, "joining an unknown BSS");
	if (!hif)
		return -ENOMEM;
	body->infrastructure_bss_mode = !vifconf->ibss_joined;
	body->short_preamble = bssconf->use_short_preamble;
	body->probe_for_join = !(channel->flags & IEEE80211_CHAN_NO_IR);
	body->channel_number = channel->hw_value;
	body->beacon_interval = cpu_to_le32(bssconf->beacon_int);
	body->basic_rate_set = cpu_to_le32(wfx_rate_mask_to_hw(wvif->wdev, bssconf->basic_rates));
	memcpy(body->bssid, bssconf->bssid, sizeof(body->bssid));
	if (ssid) {
...
int wfx_hif_start(struct wfx_vif *wvif, const struct ieee80211_vif_cfg *vifconf,
		  const struct ieee80211_bss_conf  *bssconf,
		  const struct ieee80211_channel *channel)
{
	...
	if (!hif)
		return -ENOMEM;
	body->dtim_period = bssconf->dtim_period;
	body->short_preamble = bssconf->use_short_preamble;
	body->channel_number = channel->hw_value;
	body->beacon_interval = cpu_to_le32(bssconf->beacon_int);
	body->basic_rate_set = cpu_to_le32(wfx_rate_mask_to_hw(wvif->wdev, bssconf->basic_rates));
	body->ssid_length = vifconf->ssid_len;
	memcpy(body->ssid, vifconf->ssid, vifconf->ssid_len);
