...
static int wfx_get_ps_timeout(struct wfx_vif *wvif, bool *enable_ps)
{
	...
	WARN(!wvif->vif->cfg.assoc && enable_ps,
	...
	if (chan0 && chan1 && wvif->vif->type != NL80211_IFTYPE_AP) {
		if (chan0->hw_value == chan1->hw_value) {
			...
			if (enable_ps)
				*enable_ps = false;
			if (wvif->vif->cfg.assoc && wvif->vif->cfg.ps)
	...
	if (enable_ps)
		*enable_ps = wvif->vif->cfg.ps;
	if (wvif->wdev->force_ps_timeout > -1)
		return wvif->wdev->force_ps_timeout;
	else if (wvif->vif->cfg.assoc && wvif->vif->cfg.ps)
...
static int wfx_upload_ap_templates(struct wfx_vif *wvif)
{
	...
	skb = ieee80211_beacon_get(wvif->wdev->hw, wvif->vif,0);
	if (!skb)
		return -ENOMEM;
...
{
	struct sk_buff *skb = ieee80211_beacon_get(wvif->wdev->hw, wvif->vif,0);
...
int wfx_start_ap(struct ieee80211_hw *hw, struct ieee80211_vif *vif)
{
	...
	ret = wfx_hif_start(wvif, &vif->cfg, &vif->bss_conf, wvif->channel);
	if (ret > 0)
		return -EIO;
	wfx_set_mfp_ap(wvif);
	return ret;
...
{
	int ret;
	struct ieee80211_vif_cfg *vifconf = &wvif->vif->cfg;
	struct ieee80211_bss_conf *bssconf = &wvif->vif->bss_conf;
	...
	bss = cfg80211_get_bss(wvif->wdev->hw->wiphy, wvif->channel, bssconf->bssid, NULL, 0,
			       IEEE80211_BSS_TYPE_ANY, IEEE80211_PRIVACY_ANY);
	if (!bss && !vifconf->ibss_joined) {
	...
	ret = wfx_hif_join(wvif, vifconf,bssconf, wvif->channel, ssid, ssidlen);
	if (ret) {
...
static void wfx_join_finalize(struct wfx_vif *wvif, struct ieee80211_vif *vif)
{
	struct ieee80211_vif_cfg *vifinfo = &wvif->vif->cfg;
	struct ieee80211_bss_conf *bssinfo = &wvif->vif->bss_conf;
	struct ieee80211_sta *sta = NULL;
	int ampdu_density = 0;
	bool greenfield = false;

	rcu_read_lock(); /* protect sta */
	if (bssinfo->bssid && !vifinfo->ibss_joined)
		sta = ieee80211_find_sta(wvif->vif, bssinfo->bssid);
	if (sta && sta->deflink.ht_cap.ht_supported)
		ampdu_density = sta->deflink.ht_cap.ampdu_density;
	if (sta && sta->deflink.ht_cap.ht_supported &&
	    !(bssinfo->ht_operation_mode & IEEE80211_HT_OP_MODE_NON_GF_STA_PRSNT))
		greenfield = !!(sta->deflink.ht_cap.cap & IEEE80211_HT_CAP_GRN_FLD);
	rcu_read_unlock();

	wvif->join_in_progress = false;
	wfx_hif_set_association_mode(wvif, ampdu_density, greenfield, bssinfo->use_short_preamble);
	wfx_hif_keep_alive_period(wvif, 0);
	/* beacon_loss_count is defined to 7 in net/mac80211/mlme.c. Let's use the same value. */
	wfx_hif_set_bss_params(wvif, vifinfo->aid, 7);
			  ...
			  struct ieee80211_bss_conf *info, u32 changed)
{
	...
	if (changed & BSS_CHANGED_ASSOC) {
		if (vifinfo->assoc || vifinfo->ibss_joined)
			wfx_join_finalize(wvif, vif);
		else if (!vifinfo->assoc && vif->type == NL80211_IFTYPE_STATION)
			wfx_reset(wvif);
		else
	...
	if (changed & BSS_CHANGED_ARP_FILTER) {
		for (i = 0; i < HIF_MAX_ARP_IP_ADDRTABLE_ENTRIES; i++) {
			__be32 *arp_addr = &vifinfo->arp_addr_list[i];

			if (vifinfo->arp_addr_cnt > HIF_MAX_ARP_IP_ADDRTABLE_ENTRIES)
				arp_addr = NULL;
			if (i >= vifinfo->arp_addr_cnt)
				arp_addr = NULL;
...
static int wfx_update_tim(struct wfx_vif *wvif)
{
	...
	skb = ieee80211_beacon_get_tim(wvif->wdev->hw, wvif->vif, &tim_offset, &tim_length,0);
	if (!skb)
		return -ENOENT;
